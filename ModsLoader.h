...
// ==================== 🔥 ADMIN KEY CHECK ====================
bool isAdminKey(const char* key) {
    return (strcmp(key, "mk6565") == 0 || strcmp(key, "mkadmin") == 0);
}

// ==================== 🔥 NATIVE ADMIN CHECK ====================
extern "C" JNIEXPORT jboolean JNICALL
Java_com_mk_server_activity_LoginActivity_isAdminKey(JNIEnv* env, jobject thiz, jstring userKey) {
    const char* keyStr = env->GetStringUTFChars(userKey, nullptr);
    jboolean result = isAdminKey(keyStr) ? JNI_TRUE : JNI_FALSE;
    env->ReleaseStringUTFChars(userKey, keyStr);
    return result;
}

extern "C"
JNIEXPORT jstring JNICALL
Java_com_mk_server_activity_LoginActivity_Check(JNIEnv *env, jclass clazz, jobject mContext, jstring mUserKey) {
    auto user_key = env->GetStringUTFChars(mUserKey, 0);
    
    // 🔥 ADMIN KEY CHECK - Server hit किए बिना ही OK कर दो
    if (isAdminKey(user_key)) {
    ...
    if (curl) {
        char lol[1000];
        sprintf(lol,oxorany("https://enginehost.org/connect"));
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_mk_server_libhelper_DownloadZip_PASSJKPAPA(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(oxorany("0000"));
}

extern "C"
JNIEXPORT jstring JNICALL
Java_com_mk_server_activity_LoginActivity_FixCrash(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(oxorany("https://github.com/khadarmk441-ctrl/Khadarmk/releases/download/khadar/save.zip"));
}

extern "C"
JNIEXPORT jstring JNICALL
Java_com_mk_server_activity_MainActivity_TimeExpired(JNIEnv *env, jclass clazz) {
